import { parseM3U, isValidM3U, getCategories, filterByCategory } from './m3u-parser';
...
function generateLargePlaylist(count: number): string {
  let content = '#EXTM3U\n';
  
  const categories = ['地上波', 'BS', 'CS', 'スポーツ', '映画', 'ニュース', 'アニメ', '音楽'];
  const formats = [
    // Full attributes
    (i: number, cat: string) => `#EXTINF:-1 tvg-id="${i}" tvg-name="チャンネル${i}" tvg-logo="https://example.com/logo${i}.png" group-title="${cat}",チャンネル${i}\nhttps://stream.example.com/ch${i}.m3u8`,
    // No logo
    (i: number, cat: string) => `#EXTINF:-1 tvg-id="${i}" tvg-name="チャンネル${i}" group-title="${cat}",チャンネル${i}\nhttps://stream.example.com/ch${i}.m3u8`,
    // Minimal with category
    (i: number, cat: string) => `#EXTINF:-1 group-title="${cat}",チャンネル${i}\nhttps://stream.example.com/ch${i}.m3u8`,
    // Minimal only
    (i: number, cat: string) => `#EXTINF:-1,チャンネル${i}\nhttps://stream.example.com/ch${i}.m3u8`,
  ];
...
export function runParserTests(): void {
  ...
  console.log('Valid M3U:', isValidM3U(testM3U));
  console.log('Invalid M3U:', isValidM3U('not an m3u file'));
  console.log('Empty string:', isValidM3U(''));
  console.log('');

  // Test 2: Parse M3U with various formats
  console.log('Test 2: parseM3U (Various Formats)');
  const result = parseM3U(testM3U, true); // Enable debug
  ...
  if (result.data) {
    ...
    categories.forEach(cat => {
      const count = filterByCategory(result.data!.channels, cat).length;
  ...
  const badResult = parseM3U('Invalid content');
  ...
  const largeResult = parseM3U(largePlaylist, false);
...
`;
  const edgeResult = parseM3U(edgeCases, true);
